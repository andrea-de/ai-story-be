...
class Chat {
    ...
    async fetch(messages: { role: string, content: string }[], temperature: number) {
        ...
        if (response.error) {
            Bun.write(Bun.stdout, `GPT_ERROR: ${response.error.type} - ${response.error.message} \n`);
    ...
    logTokensUsed = (task?: string) => {
        ...
        if (this.tokens <= 200) Bun.write(Bun.stdout, chalk.bold.greenBright('Tokens Used: ' + this.tokens) + taskMessage + '\n');
        else if (this.tokens <= 500) Bun.write(Bun.stdout, chalk.bold.redBright('Tokens Used: ' + this.tokens) + taskMessage + '\n');
        else Bun.write(Bun.stdout, chalk.bold.bgRedBright('Tokens Used: ' + this.tokens) + taskMessage + '\n');
    }
...
export class ChatGPTClient {
    ...
    static async generateDescription(): Promise<string> {
        try {
            const descriptionMessage = "Write a very short description of a random story in 2nd person narrative. Two sentences maximum."
            const chat = new Chat()
            const segment = await chat.send(descriptionMessage, 1.4)
