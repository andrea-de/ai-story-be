import { choiceInstruction } from "../helper/storyProgression";
...
export class ChatGPTClient {
    ...
    static async sendMessage(message: string): Promise<string> {
        try {
            // request
            // console.log("CHATGPT send message: ", message, "/n END CHATGPT")
            return '';
        } catch (error) {
    ...
    static async getStory(prompt: string): Promise<string> {
        return 'a continuation of the story from prompt: ' + prompt;
    }

    static async getChoices(prompt: string): Promise<string[]> {
